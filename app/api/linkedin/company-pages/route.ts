 ...
 * List all company pages the user has access to
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch company pages from LinkedIn API
    // LinkedIn API endpoint for organization access control
    ...
    // Get saved company pages from our database
    ...
    const pagesWithStatus = companyPages.map((page: any) => ({
      ...page,
      isConnected: savedPages?.some(s => s.company_page_id === page.id && s.is_active) || false,
    }))
 ...
 * Connect a company page for auto-posting
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { companyPageId, companyName, workspaceId } = body

    if (!companyPageId) {
      return NextResponse.json(
        { error: 'Company page ID required' },
        { status: 400 }
      )
    }
    ...
    const { data: companyPage, error: insertError } = await supabase
      ...
      .upsert({
        ...
        access_token: userData.linkedin_access_token, // In production, encrypt this
        is_active: true,
        updated_at: new Date().toISOString(),
      }, {
        onConflict: 'user_id,company_page_id',
      })
      .select()
      .single()
    ...
    if (insertError) {
      ...
      return NextResponse.json(
        { error: 'Failed to connect company page' },
        { status: 500 }
      )
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (deleteError) {
      ...
      return NextResponse.json(
        { error: 'Failed to disconnect company page' },
        { status: 500 }
      )
    }
